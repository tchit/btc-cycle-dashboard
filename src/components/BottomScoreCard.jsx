  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
        ...
        <div>
          <div style={{ fontSize: 13, color: DS.text3, letterSpacing: 2 }}>BOTTOM SCORE</div>
          <div style={{ fontSize: 15, color: DS.text2 }}>{total > 60 ? "Zone d'accumulation" : total > 40 ? 'Getting close' : 'Pas encore'}</div>
        </div>
      </div>
      {scores.map((s, i) => (
        <div key={i} style={{ display: 'grid', gridTemplateColumns: mob ? '1fr 50px 28px' : '1fr 60px 32px', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: `1px solid ${DS.borderLight}` }}>
          <div>
            <div style={{ fontSize: 14, color: DS.text2, fontWeight: 500 }}>{s.l}</div>
            <div style={{ fontSize: 13, fontFamily: DS.mono, color: DS.text3 }}>{`${s.th} → ${s.cur}`}</div>
          </div>
          ...
          <div style={{ fontSize: 13, fontFamily: DS.mono, fontWeight: 600, textAlign: 'right', color: s.st === 'hit' ? DS.up : s.st === 'partial' ? DS.warn : DS.text3 }}>

    ...
    <div style={{ display: 'grid', gridTemplateColumns: mob ? '1fr' : '1fr 1fr', gap: 20 }}>
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 11, color: DS.text3, lineHeight: 1.4 }}>Exhaustion globale</div>
        </div>
        {EXHAUST.map((e, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: `1px solid ${DS.borderLight}` }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: DS.text }}>{e.l}</div>
              <div style={{ fontSize: 10, color: DS.text3, marginTop: 2 }}>{e.d}</div>
            </div>
      ...
      <div>
        <div style={{ fontSize: 10, letterSpacing: 2, color: DS.text3, fontWeight: 600, marginBottom: 12 }}>SC\u00c9NARIOS BOTTOM</div>
        {SCENARIOS.map((s, i) => (
          <div key={i} style={{ background: s.hl ? `${s.c}08` : DS.surface2, border: `1px solid ${s.hl ? `${s.c}30` : DS.borderLight}`, borderRadius: 10, padding: '12px 14px', marginBottom: 8 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: s.hl ? 700 : 500, color: s.hl ? DS.text : DS.text2 }}>{s.l}</div>
                <div style={{ fontSize: 10, color: DS.text3, marginTop: 2 }}>{`${s.z} \u2192 ${s.r[0]}K-${s.r[1]}K`}</div>
              </div>

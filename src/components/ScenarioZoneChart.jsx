      ...
      <div key={s.l} style={{ padding: '14px 16px', borderRadius: 10, background: DS.surface, border: `1.5px solid ${inRange ? s.c : DS.border}`, position: 'relative' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            {s.hl && <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: s.c, color: '#fff', marginLeft: 4 }}>PROBABLE</span>}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <span style={{ fontSize: 13, fontFamily: DS.mono, fontWeight: 600, color: DS.text }}>{`$${s.r[0]}K \u2014 $${s.r[1]}K`}</span>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
          <div style={{ fontSize: 12, color: DS.text2 }}>{s.z}</div>
          <div style={{ display: 'flex', gap: 12 }}>
            <span style={{ fontSize: 11, fontFamily: DS.mono, color: DS.down }}>{`DD: ${s.dd[0]}% / ${s.dd[1]}%`}</span>
            {inRange && <span style={{ fontSize: 11, fontWeight: 700, color: s.c }}>ACTIF</span>}
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '8px 0' }}>
        <span style={{ fontSize: 12, color: DS.text2 }}>Prix actuel :</span>
      ...
      <div style={{ padding: '14px 16px', background: DS.borderLight, borderRadius: 10, marginTop: 4 }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: DS.text2, marginBottom: 10, textTransform: 'uppercase', letterSpacing: 0.5 }}>Niveaux de support</div>
          ...
          {keyLevels.map(l => {
            ...
            return (
              <div key={l.l} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: DS.surface, borderRadius: 8, border: `1px solid ${DS.border}` }}>
                ...
                <div>
                  <div style={{ fontSize: 12, fontWeight: 600, color: l.c }}>{l.l}</div>
                  <div style={{ fontSize: 11, fontFamily: DS.mono, color: DS.text2 }}>{`$${fP(l.v)}`}</div>
                </div>
                {below && <div style={{ marginLeft: 'auto', fontSize: 10, color: DS.up, fontWeight: 600 }}>Dessous</div>}
              </div>
            );
          })}
        </div>

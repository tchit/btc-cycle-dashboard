      ...
      <div className="card">
        ...
        <div className="card-body">
          <div style={{ display: 'grid', gridTemplateColumns: mob ? '1fr' : '1fr 1fr 1fr', gap: 16 }}>
            {[
              { l: 'Prix BTC', v: `$${fP(live.price)}`, c: DSCard.text, fk: isFake(live.fakes, 'price') },
              { l: 'Coût de minage', v: `$${fP(MC)}`, c: DSCard.warn },
              { l: 'Marge', v: `${calc.margin.toFixed(1)}%`, c: calc.margin > 0 ? DSCard.up : DSCard.down, fk: isFake(live.fakes, 'price') },
              { l: 'Difficulté', v: `${(diff / 1e12).toFixed(1)}T`, c: DSCard.text, fk: isFake(live.fakes, 'difficulty') },
              { l: 'ETF BTC Total', v: live.etfBtcTotal ? `${fP(live.etfBtcTotal)} BTC` : 'N/A', c: DSCard.accent, fk: isFake(live.fakes, 'etfBtcTotal') },
              { l: 'ETF Flow', v: live.etfFlowBtc ? `${live.etfFlowBtc > 0 ? '+' : ''}${fP(live.etfFlowBtc)} BTC` : 'N/A', c: live.etfFlowBtc > 0 ? DSCard.up : DSCard.down, fk: isFake(live.fakes, 'etfFlowBtc') }
            ].map((item, i) => (
              <div key={i} style={{ padding: '14px 16px', background: DSCard.borderLight, borderRadius: 10, border: item.fk ? '1px solid rgba(239,68,68,0.25)' : 'none' }}>
                <div style={{ fontSize: 14, color: DSCard.text3, marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.5 }}>{item.l}{item.fk && <FakeBadge />}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', background: ribbonDown ? 'rgba(204,255,0,0.06)' : 'rgba(255,0,60,0.06)', borderRadius: 10, border: `1px solid ${ribbonDown ? 'rgba(204,255,0,0.15)' : 'rgba(255,0,60,0.15)'}` }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 15, color: DSCard.text2, marginTop: 2 }}>{ribbonDown ? 'SMA30 < SMA60 — signal d\'achat historique' : 'SMA30 > SMA60 — croissance du hashrate'}</div>
                </div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div style={{ padding: '12px 14px', background: DSCard.borderLight, borderRadius: 8 }}>
                  <div style={{ fontSize: 14, color: DSCard.text3, marginBottom: 4 }}>SMA 30j</div>
                  <div style={{ fontSize: 15, fontWeight: 600, fontFamily: DS.mono }}>{`${sma30.toFixed(0)} EH/s`}</div>
                </div>
                <div style={{ padding: '12px 14px', background: DSCard.borderLight, borderRadius: 8 }}>
                  <div style={{ fontSize: 14, color: DSCard.text3, marginBottom: 4 }}>SMA 60j</div>
                  <div style={{ fontSize: 15, fontWeight: 600, fontFamily: DS.mono }}>{`${sma60.toFixed(0)} EH/s`}</div>
                </div>
              </div>
              <div style={{ fontSize: 15, color: DSCard.text2, lineHeight: 1.6 }}>{INFO.hash}</div>
            </div>
          </div>
        </div>
          ...
          <div className="card-body">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                <div style={{ fontSize: 42, fontWeight: 700, fontFamily: DS.mono, color: calc.puell < 0.5 ? DSCard.up : calc.puell > 4 ? DSCard.down : DSCard.text }}>{calc.puell.toFixed(2)}</div>
                <div style={{ fontSize: 15, color: DSCard.text3, marginTop: 4 }}>Revenu mineur / Moyenne 365j</div>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {['< 0.5 — Zone d\'achat (sous-revenue)', '0.5 - 1.0 — Recovery', '1.0 - 2.0 — Neutre', '2.0 - 4.0 — Surprofit', '> 4.0 — Zone de top'].map((t, i) => {
                  const colors = [DSCard.up, '#34d399', DSCard.text2, DSCard.warn, DSCard.down];
              ...
              <div style={{ fontSize: 15, color: DSCard.text2, lineHeight: 1.6, marginTop: 8 }}>{INFO.puell}</div>
            </div>
          </div>
        </div>
      </div>

            ...
            {SCENARIOS.map((s, i) => {
              ...
              return (
                <div key={i} style={{ padding: '18px 20px', borderRadius: 12, border: `1.5px solid ${s.hl ? s.c : DS.border}`, background: s.hl ? 'rgba(249,115,22,0.04)' : DS.surface, position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div style={{ fontSize: 15, color: DS.text2, marginBottom: 12 }}>{s.z}</div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                    <div style={{ padding: '8px 10px', background: DS.borderLight, borderRadius: 8 }}>
                      <div style={{ fontSize: 13, color: DS.text3, marginBottom: 2 }}>RANGE</div>
                      <div style={{ fontSize: 14, fontWeight: 600, fontFamily: DS.mono }}>{`$${s.r[0]}K - $${s.r[1]}K`}</div>
                    </div>
                    <div style={{ padding: '8px 10px', background: DS.borderLight, borderRadius: 8 }}>
                      <div style={{ fontSize: 13, color: DS.text3, marginBottom: 2 }}>DRAWDOWN</div>
                      <div style={{ fontSize: 14, fontWeight: 600, fontFamily: DS.mono, color: DS.down }}>{`${s.dd[0]}% / ${s.dd[1]}%`}</div>
                    </div>
                  </div>
                  {inRange && <div style={{ marginTop: 10, padding: '4px 10px', background: 'rgba(16,185,129,0.08)', borderRadius: 6, fontSize: 14, color: DS.up, fontWeight: 600, textAlign: 'center' }}>Prix actuel dans cette zone</div>}
                </div>
              );
            })}
          </div>
        ...
        <div className="card-body">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {TIMING.map((t, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 16px', background: DS.borderLight, borderRadius: 10 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 15, color: DS.text2 }}>{t.calc}</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontWeight: 700, fontSize: 14, fontFamily: DS.mono, color: DS.accent }}>{t.result}</div>
                  <div style={{ fontSize: 14, color: DS.text3 }}>{`${(t.conf * 100).toFixed(0)}% confiance`}</div>
                </div>
              </div>
            ))}
            <div style={{ marginTop: 8, padding: '12px 16px', background: 'rgba(249,115,22,0.04)', borderRadius: 10, border: '1px solid rgba(249,115,22,0.12)' }}>
              <div style={{ fontSize: 16, fontWeight: 600, color: DS.accent, marginBottom: 4 }}>Convergence: Sep - Nov 2026</div>
              <div style={{ fontSize: 15, color: DS.text2, lineHeight: 1.5 }}>Les 3 méthodes de calcul convergent vers une fenêtre de bottom potentiel entre septembre et novembre 2026.</div>
            </div>
          </div>
        </div>
      </div>

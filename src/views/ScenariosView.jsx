  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div className="stat-grid">
        <StatCard label="Distance ATH" value={`${calc.drop.toFixed(1)}%`} detail={`${calc.dATH} jours`} delta={d.price} status="neutral" fake={isFake(live.fakes, 'price')} />
        ...
        <div className="card-body">
          <div style={{ display: 'grid', gridTemplateColumns: mob ? '1fr' : '1fr 1fr', gap: 16 }}>
            {SCENARIOS.map((s, i) => {
              ...
              return (
                <div key={i} style={{ padding: '18px 20px', borderRadius: 12, border: `1.5px solid ${s.hl ? s.c : DSCard.border}`, background: s.hl ? 'rgba(204,255,0,0.04)' : DSCard.bgHover, position: 'relative', overflow: 'hidden' }}>
                  ...
                  {inRange && <div style={{ marginTop: 10, padding: '4px 10px', background: 'rgba(204,255,0,0.08)', borderRadius: 6, fontSize: 14, color: DSCard.up, fontWeight: 600, textAlign: 'center' }}>Prix actuel dans cette zone</div>}
                </div>
              );
            })}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {TIMING.map((t, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 16px', background: DSCard.borderLight, borderRadius: 10 }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: `rgba(204,255,0,${0.06 + t.conf * 0.1})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 700, fontFamily: DS.mono, color: DSCard.accent, flexShrink: 0 }}>{`${i + 1}`}</div>
                <div style={{ flex: 1 }}>
            ...
            <div style={{ marginTop: 8, padding: '12px 16px', background: 'rgba(204,255,0,0.04)', borderRadius: 10, border: '1px solid rgba(204,255,0,0.12)' }}>

        ...
        <div className="card-body">
          <div style={{ display: 'grid', gridTemplateColumns: mob ? '1fr' : '1fr 1fr', gap: 12 }}>
            {calc.bscores.filter(s => s.id === 'funding' || s.id === 'leverage').map(s => (
              <div key={s.id} style={{ padding: '14px 16px', background: DSCard.borderLight, borderRadius: 10, border: `1px solid ${s.st === 'hit' ? 'rgba(16,185,129,0.2)' : s.st === 'partial' ? 'rgba(249,115,22,0.2)' : DSCard.border}` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  <div style={{ fontWeight: 600, fontSize: 16 }}>{s.l}</div>
                  <div style={{ fontSize: 14, padding: '2px 8px', borderRadius: 10, fontWeight: 600, background: s.st === 'hit' ? 'rgba(16,185,129,0.1)' : s.st === 'partial' ? 'rgba(249,115,22,0.1)' : 'rgba(239,68,68,0.1)', color: s.st === 'hit' ? DSCard.up : s.st === 'partial' ? DSCard.accent : DSCard.down }}>{s.st === 'hit' ? 'Atteint' : s.st === 'partial' ? 'Partiel' : 'Non atteint'}</div>
                </div>

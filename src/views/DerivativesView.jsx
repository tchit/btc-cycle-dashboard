          ...
          <div className="card-body">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                ...
                <div style={{ fontSize: 16, color: DS.text2, marginTop: 6 }}>{fr < -0.01 ? 'Shorts paient les longs — Bullish' : fr > 0.05 ? 'Longs paient les shorts — Bearish' : 'Neutre'}</div>
              </div>
              ...
              <div style={{ fontSize: 15, color: DS.text2, lineHeight: 1.6, marginTop: 8 }}>{INFO.funding}</div>
            </div>
          </div>
        </div>
          ...
          <div className="card-body">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                ...
                <div style={{ fontSize: 16, color: DS.text2, marginTop: 6 }}>Valeur totale des positions futures ouvertes</div>
              </div>
              <div style={{ padding: '12px 14px', background: DS.borderLight, borderRadius: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, color: DS.text3, marginBottom: 6 }}>
                  <span>Purgé</span>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, color: DS.text3, marginTop: 6 }}>
                  <span>$0</span>
                  <span>$90B</span>
                </div>
              </div>
              <div style={{ padding: '12px 14px', background: DS.borderLight, borderRadius: 10, marginTop: 4 }}>
                <div style={{ fontSize: 14, color: DS.text3, marginBottom: 4 }}>Levier purgé vs pic</div>
                <div style={{ fontSize: 18, fontWeight: 600, fontFamily: DS.mono }}>{`${Math.max(0, (1 - oi / 90e9) * 100).toFixed(1)}%`}</div>
                <div style={{ fontSize: 14, color: DS.text3, marginTop: 2 }}>{'Cible: -40% (OI < $54B)'}</div>
              </div>
            </div>
          </div>
        </div>
            ...
            {calc.bscores.filter(s => s.id === 'funding' || s.id === 'leverage').map(s => (
              <div key={s.id} style={{ padding: '14px 16px', background: DS.borderLight, borderRadius: 10, border: `1px solid ${s.st === 'hit' ? 'rgba(16,185,129,0.2)' : s.st === 'partial' ? 'rgba(249,115,22,0.2)' : DS.border}` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  <div style={{ fontSize: 14, padding: '2px 8px', borderRadius: 10, fontWeight: 600, background: s.st === 'hit' ? 'rgba(16,185,129,0.1)' : s.st === 'partial' ? 'rgba(249,115,22,0.1)' : 'rgba(239,68,68,0.1)', color: s.st === 'hit' ? DS.up : s.st === 'partial' ? DS.accent : DS.down }}>{s.st === 'hit' ? 'Atteint' : s.st === 'partial' ? 'Partiel' : 'Non atteint'}</div>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 15, color: DS.text2 }}>
